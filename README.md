...
graph TB
    subgraph Frontend["React 19 + TypeScript Client"]
        UI[AegisVault Dashboard]
        MODALS[Deposit & Borrow Modals]
        AUDIT[Auditor Compliance Portal]
        INSPECT[Privacy Inspector]
    end

    subgraph Wallet["Wallet & Prover Layer"]
        LACE[Midnight Lace Wallet]
        FREIGHTER[Stellar Freighter Bridge]
        PROVER[Client-Side BLS12-381 ZK Prover]
        WITNESS[(Private Witness Data)]
    end

    subgraph Contract["Midnight Network (Preprod)"]
        COMPACT[Compact Smart Contract v0.19]
        CIRCUITS[4 Circuits: Deposit, Borrow, Repay, Audit]
        LEDGER[(Public Ledger: Commitments, Nullifiers, Loans)]
    end

    subgraph Regulators["Selective Compliance"]
        SEC[SEC / FINRA Examiner]
        ESMA[ESMA / MiCA Authority]
    end
